...
type Row = { cnpj: string; razao_social: string; total_importacao_fob: number };

const BarChartImport: React.FC<{ data: Row[] }> = ({ data }) => {
  return (
    <div className="rounded-xl bg-white p-4 shadow-sm">
      <h3 className="mb-2 text-base font-semibold text-slate-900">Top 10 empresas por importação</h3>
      <div className="h-96">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ left: 12 }}>
            ...
            />
            <Bar dataKey="total_importacao_fob" fill="#2563eb" />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
